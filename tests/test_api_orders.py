...
def test_full_order_lifecycle_without_preview(api_client, db, sample_template):
    r = api_client.post("/api/clients", json={"name": "Pizzaria API", "phone": "11977776666"})
    ...
    r = api_client.patch(f"/api/orders/{order['id']}", json={"quantidade": 1200})
    assert r.status_code == 200
    assert r.json()["quantidade"] == 1200

    r = api_client.get(f"/api/orders/{order['id']}")
    assert r.status_code == 200
    assert r.json()["quantidade"] == 1200


def test_create_order_requires_known_client_or_phone(api_client, sample_template):
    r = api_client.post(
        "/api/orders",
...
def test_reject_only_allowed_after_preview_sent(api_client, db, sample_client, sample_template):
    from app.db import repositories as repo

    order = repo.order_create(db, sample_client["id"], sample_template["id"], {})
    r = api_client.post(f"/api/orders/{order['id']}/reject", json={"feedback": "mudar cor"})
...
def test_full_preview_pipeline_with_real_template(api_client, db):
    ...
    r = api_client.post("/api/clients", json={"name": "Pizzaria Real", "phone": "11900001111"})
    client = r.json()
    ...
    r = api_client.get(f"/api/orders/{order['id']}/preview")
    assert r.status_code == 200
    assert r.headers["content-type"] == "image/jpeg"

    r = api_client.post(f"/api/orders/{order['id']}/approve")
    assert r.status_code == 200
    approved = r.json()
